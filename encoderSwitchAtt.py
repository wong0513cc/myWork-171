...
class ESGMultiModalModel(nn.Module):
    def __init__(self,
        ...
        self.switch = SwitchEncoder(hidden_dim=hidden, depth=6, num_heads=8, dropout=dropout)
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # 2) Cross-modal interaction with Switch Attention
        updated_list, fused = self.switch([Hp, Hf, He, Hn])  # fused: [B,K,N,H]

        # 3) 時間池化，把 [B,K,N,H] -> [B,N,H]
        #    若你想要 per-modality concat，取 updated_list 各模態做池化
        P = updated_list[0].mean(dim=1)   # price  [B,N,H]
        F = updated_list[1].mean(dim=1)   # fin    [B,N,H]
        E = updated_list[2].mean(dim=1)   # event  [B,N,H]
        Nw = updated_list[3].mean(dim=1)  # news   [B,N,H]

        # 4) 模態融合：沿最後一維 concat -> [B,N,4H]
        ...
        # 6) 整體年度（先對公司平均特徵，再回歸）→ [B,1,1]
        G = Z.mean(dim=1)                            # [B,4H]  (沿公司 N 平均)
        pred_overall = self.overall_head(G).unsqueeze(1)  # [B,1,1]
